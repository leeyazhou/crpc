...
public class ThreadPoolExecutor extends java.util.concurrent.ThreadPoolExecutor {
  ...
  /**
   * Most recent time in ms when a thread decided to kill itself to avoid potential memory leaks. Useful to throttle the
   * rate of renewals of threads.
   */
    ...
    if (currentThreadShouldBeStopped()) {
      ...
      if (lastTime + threadRenewalDelay < System.currentTimeMillis()) {
        if (lastTimeThreadKilledItself.compareAndSet(lastTime, System.currentTimeMillis() + 1)) {
          ...
          final String msg = "threadPoolExecutor.threadStoppedToAvoidPotentialLeak" + Thread.currentThread().getName();
